 ...
 */
SPITFT::SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t mosi,
		int8_t sck, int8_t rst, int8_t miso) :
		GFX(w, h), connection(TFT_SOFT_SPI), _rst(rst), _cs(cs), _dc(dc) {
	swspi._sck = sck;
	swspi._mosi = mosi;
	swspi._miso = miso;
...
#else  // !ESP8266
SPITFT::SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t rst) :
 ...
 */
SPITFT::SPITFT(uint16_t w, uint16_t h, SPIClass *spiClass, int8_t cs, int8_t dc,
		int8_t rst) :
		GFX(w, h), connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
	hwspi._spi = spiClass;
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(CORE_TEENSY)
 ...
 */
SPITFT::SPITFT(uint16_t w, uint16_t h, tftBusWidth busWidth, int8_t d0,
		int8_t wr, int8_t dc, int8_t cs, int8_t rst, int8_t rd) :
		GFX(w, h), connection(TFT_PARALLEL), _rst(rst), _cs(cs), _dc(dc) {
	tft8._d0 = d0;
	tft8._wr = wr;
	tft8._rd = rd;
 ...
 */
inline void SPITFT::writeFillRectPreclipped(int16_t x, int16_t y, int16_t w,
...
/*!
 ...
 @param  numDataBytes  The number of bytes we should send
 */
void SPITFT::sendCommand16(uint16_t commandWord, const uint8_t *dataBytes,
		uint8_t numDataBytes) {
	SPI_BEGIN_TRANSACTION();
	if (_cs >= 0)
		SPI_CS_LOW();
