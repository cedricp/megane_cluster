...
void Widget::draw(SSD1351 &display, char const *value, int text_size, int color)
{
	...
	else
	{
		if (strlen(m_title))
		{
			...
			int title_x_pos = (m_w - (strlen(m_title) * CHAR_WIDTH * text_size)) / 2;
			if (title_x_pos < 0)
				title_x_pos = 1;
			int val_x_pos = (m_w - (strlen(value) * CHAR_WIDTH * text_size)) / 2;
			if (val_x_pos < 0)
				val_x_pos = 1;
...
void Board::set_door_status(bool init)
{
	bool changed = init;
	...
	if (changed)
	{
		m_display.fillRect(54, 76, 40, 53, SSD1351::BLACK);
		m_display.drawRGBBitmap(60, 76, car_small, 28, 53);
		if (m_boot)
		{
			m_display.drawRGBBitmap(54, 76 + 40, car_small_open, 40, 33, 0, 40, 20, 33);
		}
		if (m_left_door)
		{
			m_display.drawRGBBitmap(54, 76 + 20, car_small_open, 40, 33, 0, 20, 0, 20);
		}
		if (m_right_door)
		{
			m_display.drawRGBBitmap(54, 76 + 20, car_small_open, 40, 33, 26, 40, 0, 20);
		}
	}
}

void Board::set_tires_info()
{
	char buff[5];
	...
	if (nfo.up2date)
	{
		...
		m_display.setCursor(44 - 5, 76 + 8);
		snprintf(buff, 4, "%02i%c", nfo.temp, 247);
		m_display.setTextColor(SSD1351::CYAN, SSD1351::BLACK);
		m_display.print(buff);
		nfo.up2date = false;
	}
	nfo = m_tires[1];
	...
	if (nfo.up2date)
	{
		...
		m_display.print(buff);
		m_display.setCursor(44 - 5, 120 - 8);
		snprintf(buff, 4, "%02i%c", nfo.temp, 247);
		m_display.setTextColor(SSD1351::CYAN, SSD1351::BLACK);
		m_display.print(buff);
		nfo.up2date = false;
...
void Board::set_battery()
{
	char buff[7];
	uint8_t batt = m_battery_volt.get_value();
	div_t d = div(batt, 16);
	// uint16_t decimal = batt / 16;
	// uint16_t frac = ((batt % 16) * 10) / 16;
	snprintf(buff, 5, "%02i.%i V", d.quot, d.rem * 10 / 16);
	int color = SSD1351::MAGENTA;
	if (batt > 14.5)
	{
		color = SSD1351::MAGENTA;
	}
	else if (batt > 12.2)
	{
		color = SSD1351::GREEN;
...
void Board::set_speed(bool init)
{
	char buff[4];
...
void Board::set_consumption(bool init)
{
	char buf[8];
	m_monitor.get_instant_consumption(buf);
	if (init)
...
void Board::set_cc_status()
{
	int color = SSD1351::GREEN;
	bool free = true;

	m_display.setCursor(0, 95);
	m_display.setTextSize(2);
	m_display.setTextColor(SSD1351::YELLOW, SSD1351::BLACK);
...
void Board::update(bool init)
{
	...
	uint16_t battery = m_monitor.battery_voltage_raw();
