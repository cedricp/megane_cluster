...
/* Timeout values */
#define TIMEOUT_SESSION  500 /* Timeout between successful send and receive */
...
#define MAX_MSGBUF 512    /* Received Message Buffer. Depends on uC ressources!
                             Should be enough for our needs */
struct Message_t
{
  uint16_t recv_timeout = TIMEOUT_SESSION;
  uint16_t len = 0;
...
{
  public:
    IsoTp(MCP_CAN* bus, uint8_t mcp_int = 0);
